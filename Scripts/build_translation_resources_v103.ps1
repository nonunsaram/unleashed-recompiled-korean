# Rebuild v1.0.3 reviewed dialogue archives from the verified v1.0.2 development mod.
$ErrorActionPreference='Stop'
$projectRoot=Split-Path -Parent $PSScriptRoot
$work=Join-Path $projectRoot ('Build/Translation-v103/ArchiveWork-'+[Guid]::NewGuid().ToString('N'))
$output=Join-Path $projectRoot 'Build/Translation-v103/Resources'
$baseline=Join-Path $projectRoot 'Build/Development-v102/UnleashedKorean'
...
$inputs=Get-Content -Raw (Join-Path $projectRoot 'Build/Translation-v103/resource-input.json')|ConvertFrom-Json
...
foreach($g in $inputs){
 $worldMapVariants=@{
  'BaseGame'='BaseGame';'Apotos & Shamar Adventure Pack'='ApotosShamar'
  'Chun-nan Adventure Pack'='ChunNan';'Empire City & Adabat Adventure Pack'='AllDLC'
  'Holoska Adventure Pack'='Holoska';'Mazuri Adventure Pack'='Mazuri'
  'Spagonia Adventure Pack'='Spagonia'
 }
 $isWorldMap=([string]$g.archive-eq'WorldMap')-and$worldMapVariants.ContainsKey([string]$g.package)
 $stem=if($isWorldMap){'WorldMap'}else{'+'+$g.archive}
 $relative=if($g.cutscene){'Inspire/subtitle/English'}elseif($isWorldMap){
  "WorldMapVariants/$($worldMapVariants[[string]$g.package])/Languages/English"
 ...
 if($g.cutscene){
  ...
  for($i=0;$i-lt$layout.Count;$i++){
   ...
   $charactersById.Add([int]$c.CharacterID,[string]$layout[$i].character)
  }
  ...
  function Decode([int[]]$message){
   return -join @($message|ForEach-Object {
    if($charactersById.ContainsKey([int]$_)){$charactersById[[int]$_]}else{"{GLYPH:$_}"}
   })
  }
  foreach($fg in ($g.rows|Group-Object fco_file)){
   ...
   foreach($r in $fg.Group){
    ...
    if(($cell.Message-join' ')-ne((Encode $r.before)-join' ')){
     $actual=Decode ([int[]]$cell.Message)
     $sameTextDifferentWrapping=
      (($actual-replace'\s','')-eq(([string]$r.before-replace'\s','')))
     if(-not$sameTextDifferentWrapping){throw "Baseline text mismatch: $($r.translation_key) expected=[$($r.before)] actual=[$actual]"}
    }
...
[ordered]@{passed=$true;archives=$reports;patches=$patches;game_launched=$false}|ConvertTo-Json -Depth 8|Set-Content -Encoding utf8 (Join-Path $projectRoot 'Build/Translation-v103/resource-verification.json')
