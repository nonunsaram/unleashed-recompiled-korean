"""Apply the author's v1.0.2 translation and terminology review with a keyed audit trail."""
from pathlib import Path
import json, re, shutil

R = Path(__file__).resolve().parents[1]
W = R / 'Build/Translation-v102'
CHIP_CUTSCENES = {
...
def main():
    ...
    for p in sorted((R/'Translation').glob('*-all.json')):
        ...
        for row in data['items']:
            ...
            after=(after.replace('웨어호그','웨어혹').replace('웨어혹로','웨어혹으로')
                        .replace('에그 드래군','에그 드라군'))
            if after!=before:
                edits.append({'file':p.name,'translation_key':key,'archive':row.get('archive',''),
                              ...
                              'reason':('Preserve Sonic English; exact Japanese subtitle/audio trigger match at frame 4609'
                                        ...
                                        'Correct Egg Dragoon Korean transcription; author review 2026-09-13'
                                        if '에그 드래군' in before else
    ...
    assert len(edits)==42 and sum(e['file']=='cutscenes-all.json' for e in edits)==14
    ...
    # Rebuild this scene even though its text is unchanged: the prior atlas
    # builder merged E/e keys and rendered Excellent! as ExcEllEnt!.
    scenes.add('evrt_m3_05')
    ...
    print(json.dumps({'counts':counts,'reviewed_edits':len(edits),'sonic_english_edits':1,'sonic_cutscene_latin':len(sonic),
