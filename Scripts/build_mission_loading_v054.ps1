...
$package=Join-Path $projectRoot 'Build/Korean Full UI Playtest'
$work=Join-Path $projectRoot 'Build/MissionLoadingWork-v054'
...
$inputs=Get-Content -Raw (Join-Path $projectRoot 'Build/PlayableModWork/mission-loading-input.json')|ConvertFrom-Json
...
foreach($g in $inputs){
 ...
 Get-ChildItem -LiteralPath $unpacked -File|ForEach-Object {if($_.Name-ne'StageLoad_list.fco'){$before[$_.Name]=(Get-FileHash -LiteralPath $_.FullName).Hash}}
 ...
 foreach($r in $g.rows){
  if($r.cell_index-notin@(1,2)-or[string]::IsNullOrEmpty($r.korean)){continue}
 ...
 $expected=$before.Clone();$expected['StageLoad_list.fco']=(Get-FileHash -LiteralPath (Join-Path $unpacked 'StageLoad_list.fco')).Hash
 Arc $unpacked 'hh'
 ...
 Write-Host "$($g.package)/$($g.archive): loading clone verified; $changed cells rewrapped; original files unchanged"
}
[ordered]@{passed=$true;width_including_spaces=18;wording_preserved=$true;archives=$reports;game_launched=$false}|ConvertTo-Json -Depth 8|Set-Content -Encoding utf8 (Join-Path $projectRoot 'Translation/review/mission-loading-v054/archive-verification.json')
