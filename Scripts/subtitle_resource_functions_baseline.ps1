...
function Get-GlyphWidth([string] $glyph) {
    if ($glyph.Length -eq 1 -and [int][char]$glyph -ge 33 -and [int][char]$glyph -le 126) { return [int]$latinWidths[[string][int][char]$glyph] }
    switch ($glyph) {
...
function Patch-SubtitleFile {
    ...
    for ($renderPage = 0; $renderPage -lt $atlasMaps.Count; $renderPage++) {
        ...
        & $python (Join-Path $projectRoot 'Scripts/patch_subtitle_atlas_baseline.py') $pageDdsPath $mapPath $font $pageDdsPath
