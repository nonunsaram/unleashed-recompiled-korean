...
$work=Join-Path $projectRoot 'Build/Development-v104/HMMConfigTest'
...
$results=@()
$modRoot=Join-Path $projectRoot 'Build/Development-v104/UnleashedKorean'
if($ModRootOverride){$modRoot=(Resolve-Path -LiteralPath $ModRootOverride).Path}
...
$reportName=if($ModRootOverride){'hmm-installed-config-verification.json'}else{'hmm-config-verification.json'}
@{passed=$true;modRoot=$modRoot;implementation='Actual HMM ModConfig.Load/Save and Ini';sources=$sources;choices=$results;game_launched=$false}|ConvertTo-Json -Depth 8|Set-Content -Encoding utf8 (Join-Path (Join-Path $projectRoot 'Build/Development-v104') $reportName)
