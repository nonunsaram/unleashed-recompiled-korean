"""Create the v1.0.3 GameBanana Basic and Full packages from the approved v103 mod."""
...
from configure_features_v103 import configure

R = Path(__file__).resolve().parents[1]
VERSION = "1.0.3"
SOURCE_MOD = R / "Build" / "Development-v103" / "UnleashedKorean"
PUBLIC = R / "publish" / "unleashed-recompiled-korean"
RELEASE = PUBLIC / "Release" / "v1.0.3"
...
def source_zip(full: Path) -> None:
    ...
    folders = ("Scripts", "Translation", "Patches", "Licenses", "Assets/Installer", "Assets/TitleLogo", "Tools/Fonts", "Release/v1.0.3")
    files = []
    for name in folders:
        folder = PUBLIC / name
        if folder.exists():
...
def main() -> None:
    ...
    staging = R / "Build" / ("GameBanana-v103-" + uuid.uuid4().hex)
    ...
    try:
        ...
        report = {
            ...
            "excludedExperimentalKoreanUnleashedLogo": True,
            "verification": {
        ...
        public_manifest = {key: report[key] for key in ("version", "archives", "translationArchives", "resourceFiles", "changedPhysicalCells", "unleashHDCompatibility", "setupSha256", "patchedExeSha256", "gameLaunched", "published", "defaultConfiguration", "excludedExperimentalKoreanUnleashedLogo")}
