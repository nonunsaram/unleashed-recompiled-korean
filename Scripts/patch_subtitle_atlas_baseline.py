...
def main() -> int:
    if len(sys.argv) != 5:
        print("usage: patch_subtitle_atlas_baseline.py <input.dds> <map.json> <font.ttf> <output.dds>")
        return 2
    ...
    for entry in entries:
        ...
        if "\uac00" <= glyph <= "\ud7a3" or (len(glyph)==1 and ord(glyph)<128):
            # Keep Hangul and ASCII on one shared baseline. Centering each
        ...
        # ASCII punctuation now follows the font baseline without legacy centering offsets.
        if not (len(glyph)==1 and ord(glyph)<128):
            y += entry.get("vertical_offset", 0)
        if len(glyph)==1 and ord(glyph)<128:
            assert x+left >= x0 and x+right <= x1 and y+top >= y0 and y+bottom <= y1, (glyph,entry)
