"""Verify the final v1.0.4 GameBanana archives without launching the game."""
...
def main() -> None:
    ...
    packages = {}
    work = R / "Build" / ("Package-v104-Verify-" + uuid.uuid4().hex)
    work.mkdir()
    ...
    for package in (basic, full):
        ...
        assert [x["Value"] for x in schema["Enums"]["TitleLogoVariant"]] == [
            "TitleLogos/Default", "TitleLogos/Original", "TitleLogos/Korean", "TitleLogos/Japanese", "TitleLogos/Custom"]
        ...
        for logo in ("Original", "Korean", "Japanese", "Custom"):
            assert f"TitleLogos/{logo}/+Title.ar.00" in package
            assert f"TitleLogos/{logo}/+Title.arl" in package
        assert "MODS-KO.md" in package
    approved = json.loads((R / "Build/TitleLogo-Choices/verification.json").read_text(encoding="utf8"))
    for package in (basic, full):
        assert not any(name.startswith("TitleLogos/Default/") for name in package)
    ...
    with zipfile.ZipFile(io.BytesIO(full["Source.zip"])) as source:
        ...
        assert "Source/Release/v1.0.4/manifest.json" not in source.namelist()
        ...
        assert source.read("Source/Scripts/package_hmm_release_v104.py") == (R / "publish/unleashed-recompiled-korean/Scripts/package_hmm_release_v104.py").read_bytes()
        assert source.read("Source/Scripts/verify_package_v104.py") == (R / "publish/unleashed-recompiled-korean/Scripts/verify_package_v104.py").read_bytes()
