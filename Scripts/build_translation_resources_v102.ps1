...
$work=Join-Path $projectRoot ('Build/Translation-v102/ArchiveWork-'+[Guid]::NewGuid().ToString('N'))
$output=Join-Path $projectRoot 'Build/Translation-v102/Resources'
...
$inputs=Get-Content -Raw (Join-Path $projectRoot 'Build/Translation-v102/resource-input.json')|ConvertFrom-Json
  ...
  foreach($fg in ($g.rows|Group-Object fco_file)){
   ...
   foreach($r in $fg.Group){
    ...
    if(($cell.Message-join' ')-ne((Encode $r.before)-join' ')){
     ...
     $sameEggDragoonText=([string]$r.before).Contains('에그 드래군') -and
      (($actual-replace'\s','')-eq(([string]$r.before-replace'\s','')))
     if(-not$sameEggDragoonText){throw "Baseline text mismatch: $($r.translation_key)"}
    }
...
[ordered]@{passed=$true;archives=$reports;patches=$patches;game_launched=$false}|ConvertTo-Json -Depth 8|Set-Content -Encoding utf8 (Join-Path $projectRoot 'Build/Translation-v102/resource-verification.json')
